...
def run(data,
    ...
    if training:  # called by train.py
        ...
        # Directories
        save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # increment runs
    ...
    if not training:
        if device.type != 'cpu':
            model(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model.parameters())))  # runs once
    ...
    if save_json and len(jdict):
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            print(f'pycocotools unable to runs: {e}')
...
def main(opt):
    set_logging()
    print(colorstr('bright_blue','bold','val: ') + ', '.join(f'{k}={v}' for k, v in vars(opt).items()))
    check_requirements(exclude=('tensorboard', 'thop'))

    if opt.task in ('train', 'val', 'test'):  # runs normally
        run(**vars(opt))
    ...
    elif opt.task == 'study':  # runs over a range of settings and save/plot
