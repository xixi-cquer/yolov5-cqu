...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        run_id = run_id if opt.resume else None  # start fresh runs if transfer learning
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        logging.warning('DP not recommended, instead use torch.distributed.runs for best DDP Multi-GPU results.\n'
...
def main(opt):
    ...
    if opt.resume and not wandb_run:  # resume an interrupted runs
...
def run(**kwargs):
    # Usage: import train; train.runs(imgsz=320, weights='yolov5m.pt')
    opt = parse_opt(True)
